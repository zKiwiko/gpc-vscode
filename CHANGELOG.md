...
- **Added `ps5adt` data type (but not really...)**
  - **Known Issue:** LSP reports back a syntax error from the Parser. The original grammar file used to generate it was lost, so it will always report back as a syntax until a replacement is ready to be used.
